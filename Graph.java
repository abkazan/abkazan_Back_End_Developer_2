...
public class Graph<T> implements GraphADT<T> {
  private Hashtable<String, String> backpack = new Hashtable(); // player's backpack
  private File riddles = new File(""); // csv file with all the riddles
  /**
   * gets the player's backpack of items
   * @return a hashtable of the player's backpack
   */
  public Hashtable<String, String> getBackpack() {
    return this.backpack;
  }
  /**
   * gets the file of riddles
   * @return the riddle file
   */
  public File getRiddles() {
    return this.riddles;
  }
  /**
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    Hashtable<Vertex, Path> visited = new Hashtable<>();
    pq.add(p);
    while (!pq.isEmpty()) {
      Path curr = (Graph<T>.Path) pq.poll();
      if (curr.end.data.equals(end)) {
        return curr;
      }
      visited.put(curr.end, curr);
      for (Edge e : curr.end.edgesLeaving) {
        if (!visited.containsKey(e.target)) {
          Path newPath = new Path(curr);
          newPath.extend(e);
          pq.add(newPath);
        }
      }
    }
    throw new NoSuchElementException();// TODO: Implement this method in Step 8.
